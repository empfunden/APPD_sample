# Python implementation of the cortical 5D model from summer 2023.
# M derivative calculation uses average of front/back level set points; *same as described in APPD paper*.
...
def single_update_plot(p_list, t):
    ...
    p1.imshow(density_data, cmap=colormaps["magma"])
...
if __name__ == "__main__":
    ...
    output_name = f"APPD-output_{diff_coeff:.2e}_{coupling_coeff:.2e}_{max_steps}.npz"
    time_array = np.array([total_time]) # just one number (total time of the simulation)
    pcount_array = np.array(particle_history)
    appcur_array = np.array([appcur])
    diff_array = np.array([diff_coeff])
    coup_array = np.array([coupling_coeff])
    np.savez(output_name, time_array=time_array, pcount_array=pcount_array, appcur_array=appcur_array,
    ...
    plot_V.grid()
    plt.savefig(f"APPD-voltage_{appcur:.2e}_{diff_coeff:.2e}_coupling_{coupling_coeff:.2e}_{max_steps}.png")
    plt.close()
