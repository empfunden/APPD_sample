...
def HH_advection_velocity(X): # five-dimensional
    dXdt = np.empty([5])

    V = 100.0 * X[0]
    M, N, H, P = X[1], X[2], X[3], X[4]
    V += 1e-6 * int(
        abs(V - 10.0) < 5e-7 or abs(V - 25.0) < 5e-7 or abs(V - 50.0) < 5e-7
    )  # don't want V = 10 or 25 or 50
    if N > 1.0 or N < 0 or H > 1.0 or H < 0 or P > 1.0 or P < 0:
        print("V, M, N, H, P:", V, M, N, H, P)
    try:
        Ah = 0.128 * math.exp(-(V - Vt - 17.0) / 18.0)
        Am = 0.32 * (V - Vt - 13.0) / (1 - math.exp(-(V - Vt - 13.0) / 4.0))
        An = 0.032 * (V - Vt - 15.0) / (1 - math.exp(-(V - Vt - 15.0) / 5.0))
        Bh = 0.5 / math.exp(-(V - Vt - 10.0) / 40.0)
        Bm = 0.28 * (V - Vt - 40.0) / (math.exp((V - Vt - 40.0) / 5.0) - 1.0)
        Bn = 0.5 * math.exp(-(V - Vt - 10.0) / 40.0)

        p_inf = 1.0 / (1.0 + math.exp(-(V + 35.0) / 10.0))
        tau_p = tau_max / (3.3 * math.exp((V + 35.0) / 20.0) + math.exp(-(V + 35.0) / 20.0))

        dXdt[1] = Am * (1 - M) - Bm * M
        dXdt[2] = An * (1 - N) - Bn * N
        dXdt[3] = Ah * (1 - H) - Bh * H
        dXdt[4] = (p_inf - P) / tau_p

        I_L = gl * (V - el)
        I_Na = gna * pow(M, 3) * H * (V - ena)
        I_K = gk * pow(N, 4) * (V - ek)
        I_KS = gm * P * (V - ek)

        dXdt[0] = -(appcurr + I_L + I_Na + I_K + I_KS) / 100.0
    except OverflowError as e:
        raise OverflowError("Overflow error. Location is ", X)
...
def HH_advection_velocity_G(t, G):
    ...
    try:
        ...
        raise OverflowError("Overflow in HH_advection_velocity(X) raised to velocity_G")
...
def upaiss(timestep: float, p: Particle):
    ...
    while (not did_once) or (adjustment > minadjustment):
        ...
        try:
            ...
            # See code in upai for when remaining time is -1.
            print("Overflowed during integration at G=", G, "so we will delete the particle.")
            return False, -1, np.zeros(DIM)
...
def upai(timestep, coupling_timestep, p):
    ...
    if remaining_time == -1:
        # New code to delete overflowed particles
        global deleted_weight
        deleted_weight += p.weight
        return 0
...
def update_ODE_adaptive_split(timestep):
    ...
    # Want to normalize particles (since some may have been deleted because of overflow)
    global deleted_weight
    if deleted_weight > 0:
        normfactor = 1.0 / (1.0 - deleted_weight)
        for p in particle_list:
            p.weight *= normfactor
    # Reset it for next timestep
    deleted_weight = 0
...
v1, v2 = np.array([1, 0, 0, 0, 0]), np.array([0, 0, 1, 0, 0])
    ...
    def shot(z1):
        ...
        for t, y in integrate(h, y0, HH_advection_velocity):
            ...
            if y2[0] < 0 and y3[0] >= 0:
                zfA = lagrange([y1[0], y2[0], y3[0], y4[0]], [y1[1], y2[1], y3[1], y4[1]])  # y[1]
                zfB = lagrange([y1[0], y2[0], y3[0], y4[0]], [y1[2], y2[2], y3[2], y4[2]])  # y[2]
                zfC = lagrange([y1[0], y2[0], y3[0], y4[0]], [y1[3], y2[3], y3[3], y4[3]])  # y[3]
                zfD = lagrange([y1[0], y2[0], y3[0], y4[0]], [y1[4], y2[4], y3[4], y4[4]])  # y[4]
                tf = lagrange([y1[0], y2[0], y3[0], y4[0]], [t1, t2, t3, t4])
                return tf(0), np.array([0, zfA(0), zfB(0), zfC(0), zfD(0)])

    z = np.array([0,0.0501172,0.40763994,0.44179683,0.5])
    for i in range(100):
        TT, zn = shot(z)
        dz = zn - z
        z = zn    
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 4: # If we have save file:
        ...
        particle_list[0].mu = np.array([0.5, 0.5, 0.5, 0.5, 0.5])
    ...
    output_name = f"particlecounts_{diff_coeff:.2e}_{coupling_coeff:.2e}_{max_steps}.npz"
    ...
    summary = f"diff_coeff={diff_coeff:.2e}, coupling_coeff={coupling_coeff:.2e}, simulated {max_steps} steps in {total_time:.2f} seconds, avg particle count={np.mean(pcount_array):.2f}"

    print(summary)

    fig = plt.figure()
    p1 = fig.add_subplot(1, 1, 1)
    p1.set_title(summary)
    # p1.plot(particle_history)
    p1.plot(voltage_record)
    p1.grid()
    # plt.savefig(f"particlecounts_{diff_coeff:.2e}_{coupling_coeff:.2e}_{max_steps}.jpeg")
    plt.savefig(f"voltage_{diff_coeff:.2e}_{coupling_coeff:.2e}_{max_steps}.jpeg")

    # video_name = "hh5d_" + str(date.today()) + "_" + str(time.time()) + ".mp4"
    # subprocess.call(
    #     [
    #         "ffmpeg",
    #         "-framerate",
